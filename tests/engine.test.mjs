...
test("balance report highlights death causes, floor bands, and build performance", () => {
  ...
  assert.equal(typeof report.finalBossAnalysis.pressureProfile.sequenceLosses, "number");
  assert.equal(typeof report.finalBossAnalysis.pressureProfile.averageRequiemHandBurstDefense, "number");
  assert.equal(typeof report.finalBossAnalysis.rewardGuidance.defensiveShare, "number");
  assert.equal(typeof report.finalBossAnalysis.rewardGuidance.defenseWhileFinishMissing, "number");
  assert.equal(typeof report.finalBossAnalysis.reserveSignals.averageSignalsPerReached, "number");
  assert.equal(typeof report.finalBossAnalysis.reserveSignals.maxSignalsPerRun, "number");
  assert.equal(typeof report.finalBossAnalysis.roleAverages.wins.burstDefense, "number");
  assert.ok(Array.isArray(report.finalBossAnalysis.byDifficulty));
  assert.ok(report.finalBossAnalysis.byDifficulty.every((entry) => typeof entry.averageEntryHpRatio === "number"));
  assert.ok(report.finalBossAnalysis.byDifficulty.every((entry) => typeof entry.noBurstDefenseAtRequiemLosses === "number"));
  assert.ok(report.runs.every((run) => Array.isArray(run.finalBossTimeline)));
  assert.ok(report.runs.every((run) => typeof run.finalBossRewardSampleCount === "number"));
  assert.ok(report.runs.every((run) => typeof run.finalBossReserveSignalCount === "number"));
  assert.ok(report.runs.every((run) => !("finalBossRewardSamples" in run)));
  assert.ok(report.runs.every((run) => !("finalBossReserveSignals" in run)));
