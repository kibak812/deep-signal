...
test("release documentation lists QA artifacts and current combat feedback", () => {
  ...
  assert.match(readme, /런 포기 확인과 포기 요약/);
  ...
  assert.match(playtestSource, /danger-confirmations/);
  ...
  assert.match(playtestSource, /dangerConfirmations/);
  assert.match(playtestSource, /abandonRun\(run\)/);
  ...
  assert.match(readme, /browser-qa-abandon-confirm\.png/);
  assert.match(readme, /browser-qa-abandon-summary\.png/);
  assert.match(readme, /browser-qa-abandon-run\.json/);
  ...
  assert.match(captureSource, /railEnergySprite/);
  ...
  assert.match(captureSource, /browser-qa-abandon-confirm\.png/);
  assert.match(captureSource, /browser-qa-abandon-summary\.png/);
...
test("accessibility settings and combat feedback are wired into the rendered UI", () => {
  ...
  assert.match(mainSource, /<span class="\$\{cardUiIconClass\(primary\.iconKey\)\}" aria-hidden="true"><\/span>/);
  assert.match(mainSource, /<b class="\$\{cardUiIconClass\(chip\.iconKey\)\}" aria-hidden="true"><\/b>/);
  assert.match(mainSource, /iconKey:\s*"damage"/);
  assert.match(mainSource, /iconKey:\s*"block"/);
  assert.match(mainSource, /iconKey:\s*"status"/);
  assert.match(mainSource, /value:\s*`-\$\{forecast\.hpLoss\}`/);
  assert.doesNotMatch(mainSource, /value:\s*`피해 \$\{forecast\.hpLoss\}`/);
  assert.match(mainSource, /function combatForecastSetupIconKey\(forecast\)/);
  ...
  assert.match(mainSource, /function statusIconClass\(keyword\)/);
  assert.doesNotMatch(mainSource, /function statusIcon\(keyword\)/);
  ...
  assert.match(mainSource, /<i class="\$\{statusIconClass\(key\)\}" aria-hidden="true"><\/i><strong>\$\{value\}<\/strong>/);
  ...
  assert.match(mainSource, /data-status-key="more"/);
  ...
  assert.match(mainSource, /function enemyIntentIconClass\(move = \{\}\)/);
  assert.match(mainSource, /<span class="intent-tooltip-icon \$\{enemyIntentIconClass\(move\)\}" aria-hidden="true"><\/span>/);
  ...
  assert.match(mainSource, /<b class="\$\{cardUiIconClass\("damage"\)\}"><\/b><em>-\$\{fxHitAmount\}<\/em>/);
  assert.match(mainSource, /<b class="\$\{cardUiIconClass\("block"\)\}"><\/b><em>-\$\{fxBlockLossAmount\}<\/em>/);
  ...
  assert.match(mainSource, /return `<i class="\$\{chip\.tone\}" title="\$\{chip\.label\}"><b class="\$\{cardUiIconClass\(visual\.key\)\}"><\/b><em>\$\{visual\.value\}<\/em><\/i>`/);
  ...
  assert.match(styleSource, /status-icons\.png/);
  assert.match(styleSource, /background-size:\s*2000% 100%/);
  assert.match(styleSource, /\.status-icon-vulnerable[\s\S]*--status-icon-position:\s*0% 50%/);
  assert.match(styleSource, /\.status-icon-more[\s\S]*--status-icon-position:\s*100% 50%/);
  ...
  assert.match(mainSource, /class="preview-energy-after"[\s\S]*title="사용 후 남은 전하 \$\{Math\.max\(0, preview\.energyAfter\)\}"[\s\S]*<span class="preview-energy-icon \$\{cardUiIconClass\("energy"\)\}"><\/span><em>\$\{Math\.max\(0, preview\.energyAfter\)\}<\/em><\/b>/);
  ...
  assert.match(styleSource, /\.preview-energy-after \.card-ui-icon[\s\S]*width:\s*17px/);
  assert.match(styleSource, /\.card-ui-icon-energy[\s\S]*--card-ui-icon-position:\s*61\.5385% 50%/);
  assert.doesNotMatch(styleSource, /\.preview-energy-after \.preview-energy-icon::before[\s\S]*content:\s*"⚡"/);
  ...
  assert.match(mainSource, /<em>다음 행동 · \$\{intentVisualLabel\}<\/em>/);
  ...
  assert.match(styleSource, /\.intent[\s\S]*width:\s*min\(206px, calc\(100% - 16px\)\)/);
  ...
  assert.match(styleSource, /@media \(min-width: 1041px\)[\s\S]*\.enemy-line\.enemy-count-1 \.enemy-card \.intent[\s\S]*width:\s*100px/);
  assert.match(styleSource, /@media \(min-width: 1041px\)[\s\S]*\.enemy-line\.enemy-count-1 \.enemy-card \.intent[\s\S]*height:\s*60px/);
  assert.match(styleSource, /@media \(min-width: 1041px\)[\s\S]*\.enemy-line\.enemy-count-1 \.enemy-card \.intent > span[\s\S]*height:\s*45px/);
  assert.match(captureSource, /singleEnemyIntentDock/);
  assert.match(captureSource, /singleEnemyIntentDock\.ready/);
  assert.match(captureSource, /compactSize/);
  assert.match(captureSource, /textStackTall/);
...
test("title screen supports seeded and daily challenge runs with Korean credits", () => {
  ...
  assert.match(mainSource, /pendingAbandonRun:\s*null/);
  ...
  assert.match(mainSource, /function requestAbandonRun\(\)/);
  assert.match(mainSource, /function abandonCurrentRunNow\(\)/);
  assert.match(mainSource, /function abandonCurrentRunNow\(\)[\s\S]*state\.screen = "game"/);
  ...
  assert.match(styleSource, /\.abandon-confirm/);
...
test("combat hand cards expose play outcome previews", () => {
  ...
  assert.match(mainSource, /if \(\/처치\/\.test\(label\)\) return \{ key: "damage", icon: "✕", value: number \|\| "끝" \}/);
  assert.match(mainSource, /key: "damage", icon: "✦"/);
  assert.match(mainSource, /key: "block", icon: "⬡"/);
  assert.match(mainSource, /key: "draw", icon: "▤"/);
  assert.match(mainSource, /key: "energy", icon: "⚡"/);
  assert.match(mainSource, /function cardUiIconClass\(key\)/);
  assert.match(mainSource, /function cardOutcomeIconClass\(visual = \{\}\)/);
  assert.match(mainSource, /card-ui-icon-\$\{safeKey\}/);
  assert.match(mainSource, /class="\$\{cardOutcomeIconClass\(visual\)\}" aria-hidden="true"><\/b>/);
  ...
  assert.match(styleSource, /card-ui-icons\.png/);
  assert.match(styleSource, /background-size:\s*1400% 100%/);
  assert.match(styleSource, /\.card-ui-icon-damage[\s\S]*38\.4615% 50%/);
  assert.match(styleSource, /\.card-outcome \.card-ui-icon[\s\S]*width:\s*18px/);
