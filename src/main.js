...
function bossPreparationServiceAdvice(run, options = {}) {
  ...
  const needsRole = missing.some((label) => ["방어", "마무리", "정화·약화"].includes(label));
...
function bossPreparationRestAdvice(run, options = {}) {
  ...
  if (missing.some((label) => ["방어", "마무리", "정화·약화"].includes(label)) && options.canUpgrade) {
    return {
      tone: "strong",
      title: "보스용 카드 강화",
...
function handLayoutStyle(count) {
  ...
  return `--hand-count:${count}; --hand-overlap:${overlap}px;`;
}
